...
Ext.define("AP.view.dataMaintaining.CalculateMaintainingInfoView", {
    ...
    initComponent: function () {
        var me = this;
        var calculateResultStore=Ext.create('AP.store.dataMaintaining.CalculateManagerDataStore');
        var CalculateManagerWellListStore=Ext.create('AP.store.dataMaintaining.CalculateManagerWellListStore');
...
//                		showWellName+='井';
                ...
                handler: function (v, o) {
                	if(calculateManagerHandsontableHelper.hot.getSelected()){
                		var row=calculateManagerHandsontableHelper.hot.getSelected()[0][0];
                		var recordId=calculateManagerHandsontableHelper.hot.getDataAtRow(row)[0];
...
//        	}
        		...
        		columns+=",type:'dropdown',strict:true,allowInvalid:false,source:['','A','B','C','D','K','KD','HL','HY'], validator: function(val, callback){return handsontableDataCheck_RodGrade(val, callback,this.row, this.col,calculateManagerHandsontableHelper);}";
        	}else{
